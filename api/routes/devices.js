...
async function createSaverRule(userId, dId, status) {
  ...
  try {
    const url = "http://"+process.env.EMQX_API_HOST+":8081/api/v4/rules";
...
async function updateSaverRuleStatus(emqxRuleId, status) {
  try {
    ...
    const url = "http://"+process.env.EMQX_API_HOST+":8081/api/v4/rules/" + emqxRuleId;
...
async function deleteSaverRule(dId) {
  try {
    ...
    const url = "http://"+process.env.EMQX_API_HOST+":8081/api/v4/rules/" + mongoRule.emqxRuleId;
...
async function deleteAllAlarmRules(userId, dId) {
  try {
    ...
    const url = "http://"+process.env.EMQX_API_HOST+":8081/api/v4/rules/" + rule.emqxRuleId;
