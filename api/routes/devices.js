...
async function createSaverRule(userId, dId, status) {
  ...
  try {
    const url = "http://"+process.env.EMQX_API_HOST+":8091/api/v4/rules";
...
async function updateSaverRuleStatus(emqxRuleId, status) {
  try {
    const url = "http://"+process.env.EMQX_API_HOST+":8091/api/v4/rules/" + emqxRuleId;
...
async function deleteSaverRule(dId) {
  try {
    ...
    const url = "http://"+process.env.EMQX_API_HOST+":8091/api/v4/rules/" + mongoRule.emqxRuleId;
  ...
  try {
    ...
    if (rules.length > 0) {
      asyncForEach(rules, async rule => {
        const url = "http://"+process.env.EMQX_API_HOST+":8091/api/v4/rules/" + rule.emqxRuleId;
